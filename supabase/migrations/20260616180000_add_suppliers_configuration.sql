-- Migration to add Suppliers configuration and associate with spare_parts

-- 1. Create spare_part_suppliers table
...
-- Enable RLS
ALTER TABLE public.spare_part_suppliers ENABLE ROW LEVEL SECURITY;

-- Drop existing policies if any
DROP POLICY IF EXISTS "Public Read Suppliers" ON public.spare_part_suppliers;
DROP POLICY IF EXISTS "Public Write Suppliers" ON public.spare_part_suppliers;

-- RLS Policies
CREATE POLICY "Public Read Suppliers" ON public.spare_part_suppliers FOR SELECT USING (true);
CREATE POLICY "Public Write Suppliers" ON public.spare_part_suppliers FOR ALL USING (true) WITH CHECK (true);

-- Grants
GRANT ALL ON TABLE public.spare_part_suppliers TO anon, authenticated, service_role;

-- 2. Add supplier column to spare_parts if not exists (already added in coreflow_upgrade_v2.sql but just in case)
ALTER TABLE public.spare_parts ADD COLUMN IF NOT EXISTS supplier text;

-- Drop the old 23-parameter function signature
DROP FUNCTION IF EXISTS public.upsert_spare_part(text, text, text, text, text, text, numeric, numeric, numeric, text, text, numeric, text, timestamp with time zone, text, text, text, text, text, text, text, text, text);

-- Recreate the function with new p_supplier parameter
...
BEGIN
    ...
    ELSE
        ...
        INSERT INTO public.spare_parts (id, sku, name, description, category, unit_of_measure,
                                         ...
                                         machine_model, machine_line, supplier)
        VALUES (p_id, p_sku, p_name, p_description, p_category, p_unit_of_measure,
                ...
                p_machine_model, p_machine_line, p_supplier)
        RETURNING *;
    END IF;
END;
$$;
