-- Migration to add supplier and machine association fields to spare_parts table
ALTER TABLE public.spare_parts ADD COLUMN IF NOT EXISTS machine_plate text;
ALTER TABLE public.spare_parts ADD COLUMN IF NOT EXISTS machine_name text;
ALTER TABLE public.spare_parts ADD COLUMN IF NOT EXISTS catalog text;
ALTER TABLE public.spare_parts ADD COLUMN IF NOT EXISTS table_no text;
ALTER TABLE public.spare_parts ADD COLUMN IF NOT EXISTS figure text;
ALTER TABLE public.spare_parts ADD COLUMN IF NOT EXISTS supplier_code text;

-- Drop the old 15-parameter function signature
DROP FUNCTION IF EXISTS public.upsert_spare_part(text, text, text, text, text, text, numeric, numeric, numeric, text, text, numeric, text, timestamp with time zone, text);
